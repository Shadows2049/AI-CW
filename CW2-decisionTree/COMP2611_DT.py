...
def deviation(value,parent_pos,parent_neg):
    ...
    #Insert code here
    expect_pos = parent_pos * (value.pos + value.neg) / (parent_pos + parent_neg)
    expect_neg = parent_neg * (value.pos + value.neg) / (parent_pos + parent_neg)
    deviation = (value.pos - expect_pos) ** 2 / expect_pos + (value.neg - expect_neg) ** 2 / expect_neg
    return (deviation)
    ...
    def decisiontree_iterator(parent):
        ...
        if further_fork == False:
            ...
            if(parent.pos > 0 and parent.neg > 0):
                ...
                p_value = 1 - stats.chi2.cdf(DELTA, df = len(parent.branches)-1)
...
def test_tennis_tree(filename):
    ...
    #insert code here
    dataSet = DataSet(name=filename, target='Play')
    train,test = train_test_split(dataSet, test_split=0.2)
    trainSet = train
    testSet = test
...
def train_restaurant_tree(trainSet, testSet, N=200):
    ...
    #insert code here
    treeE = DecisionTreeLearner(trainSet)
    errorE = err_ratio(treeE, testSet)
    samples_required = N
    for i in range(1,N+1):
        train = SyntheticRestaurant(i)
        tree = DecisionTreeLearner(train)
        error = err_ratio(tree, testSet)
        if error == errorE:
            if i <= samples_required:
                samples_required = i
                ##print("minimal sample required is ",i)
        
    samples_required = samples_required - 1 ##gradescope possible error
